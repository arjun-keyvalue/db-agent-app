...
class DatabaseConnection:
    ...
    def connect(
        ...
        try:
            ...
            # Create engine
            logger.debug(f"Creating database engine with URL: {connection_url}")
            ...
            logger.debug(f"Database connection test successful")
    ...
    def get_tables(self) -> Tuple[bool, Any]:
        ...
        try:
            if self.connection_info["type"] == "postgresql":
                ...
                return (
                    False,
                    f"Unsupported database type: {self.connection_info['type']}",
                )
    ...
    def _auto_index_schema(self):
        ...
        try:
            ...
            if not Config.AUTO_INDEX_SCHEMA:
                logger.debug("Auto-indexing disabled (AUTO_INDEX_SCHEMA=false)")
                return
            ...
            if success:
                ...
                logger.warning(
                    "⚠ Schema indexing failed, but database connection is still active"
                )
                logger.info(
                    "💡 You can manually test indexing with: python test_embedding_system.py"
                )

        except ImportError as e:
            logger.warning(f"Schema indexing dependencies not available: {str(e)}")
            logger.info(
                "💡 Install missing dependencies: pip install lancedb sentence-transformers"
            )
        except Exception as e:
