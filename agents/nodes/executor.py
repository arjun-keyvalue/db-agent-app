        ...
        try:
            ...
            if success:
                ...
                if hasattr(result, 'shape'):
                    logger.info(f"Result: {result.shape[0]} rows, {result.shape[1]} columns")
                ...
                # Determine if this is correctable
                if self._is_correctable_error(str(result)):
                    state['next_action'] = 'self_correction'
                    state['needs_correction'] = True
                    logger.warning(f"Query execution failed (correctable): {result}")
                else:
                    state['error_message'] = f"Query execution failed: {result}"
                    state['next_action'] = 'output_formatter'
                    state['success'] = False
                    logger.error(f"Query execution failed (non-correctable): {result}")
            ...
            # Most exceptions are correctable
            if self._is_correctable_error(str(e)):
                state['next_action'] = 'self_correction'
                state['needs_correction'] = True
            else:
