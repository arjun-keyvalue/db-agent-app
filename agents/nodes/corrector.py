...
class SelfCorrectionNode:
    ...
    def __call__(self, state: AgentState) -> AgentState:
        ...
        if correction_attempts >= max_corrections:
            state['error_message'] = f"Maximum correction attempts ({max_corrections}) exceeded"
        ...
        if not state.get('has_limit_clause', True):
            feedback_parts.append("Performance Issue: Query lacks LIMIT clause for potentially large result sets")
        ...
        try:
            # Use LiteLLM for model flexibility
            response = litellm.completion(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert SQL developer with strong debugging skills. Your job is to fix SQL queries based on error feedback."},
            ...
            logger.info(f"Self-correction attempt {state['correction_attempts']} completed")
    ...
    def _create_correction_prompt(self, user_query: str, current_sql: str, database_schema: str, 
                                feedback: str, database_type: str, attempt_number: int) -> str:
