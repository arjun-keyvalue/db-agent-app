...
class PerformanceGuardNode:
    ...
    def __call__(self, state: AgentState) -> AgentState:
        ...
        # 5. Check for dangerous operations - be less strict
        dangerous_operations = self._check_dangerous_operations(sql_query)
        correction_attempts = state.get("correction_attempts", 0)
        max_corrections = state.get("max_corrections", 3)

        if dangerous_operations and correction_attempts < max_corrections - 1:
            # Only block if we haven't tried corrections yet
            state["error_message"] = (
                f"Potentially dangerous operations detected: {', '.join(dangerous_operations)}"
            )
            state["next_action"] = "self_correction"
            state["validation_errors"] = [
                f"Dangerous operation: {op}" for op in dangerous_operations
            ]
            return state
        elif dangerous_operations:
            # Log warning but proceed if we've tried corrections
            logger.warning(
                f"Proceeding with potentially dangerous operations after corrections: {dangerous_operations}"
            )
            state["validation_errors"] = state.get("validation_errors", []) + [
                f"Warning: {op}" for op in dangerous_operations
            ]
