...
class OutputFormatterNode:
    ...
    def _format_success_output(
        self, result, sql_query: str, attempts: int, exec_time: float
    ) -> str:
    ...
    def _format_error_output(
        self, error_message: str, sql_query: str, attempts: int
    ) -> str:
        """Format error output"""
        ...
        if attempts > 0:
            output_parts.append(
                f"❌ Query failed after {attempts} correction attempt(s)"
            )
        else:
            output_parts.append("❌ Query generation/execution failed")

        # Add error message
        output_parts.extend(["", "Error Details:", error_message])

        # Add SQL query if available
        if sql_query:
            output_parts.extend(
                ["", "🔍 Last Generated SQL Query:", f"```sql\n{sql_query}\n```"]
            )
