...
class SmartLLMClient:
    ...
    def get_model_name(self) -> str:
        ...
        if self.provider == "openai":
            return self.model  # OpenAI models don't need provider prefix
        elif self.provider == "ollama":
            return f"ollama/{self.model}"
        elif self.provider == "groq":
            # Groq needs proper formatting
            return f"groq/{self.model}"
        else:
            # For other providers, include provider name in model string
    ...
    def _setup_api_keys(self):
        ...
        if provider == "openai":
            ...
            api_key = (
                self.api_key or Config.ANTHROPIC_API_KEY or Config.get_rag_api_key()
            )
            if api_key:
    ...
    def completion(self, messages: List[Dict[str, str]], **kwargs) -> Any:
        ...
        # Get the proper model name for LiteLLM
        model_name = self.get_model_name()

        # Default parameters
        default_params = {
            "model": model_name,  # Use the formatted model name
            "messages": messages,
        ...
        try:
            ...
            raise Exception(
                f"Rate limit exceeded for {self.provider}. Please try again later."
            ) from e

        except Exception as e:
            if "LLM Provider NOT provided" in str(e):
                error_msg = f"LLM Provider formatting error. Check the model and provider settings. Model: {self.model}, Provider: {self.provider}"
    ...
    def get_embedding(
        ...
        try:
            if self.provider == "openai":
                ...
                logger.warning(
                    f"Embeddings not directly supported for {self.provider}, falling back to OpenAI"
                )
                ...
                if openai_key:
                    ...
                    raise Exception(
                        "OpenAI API key required for embeddings when using non-OpenAI providers"
                    )
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if response and response.choices:
                ...
                logger.error(
                    f"LLM connection test failed for {self.provider}: No response"
                )
                return False
