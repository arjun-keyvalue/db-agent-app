...
class RAGAgent(BaseAgent):
    ...
    def __init__(
        ...
        if Config.AUTO_INDEX_SCHEMA and self.schema_indexer.db:
            logger.debug("Auto-indexing database schema...")
            try:
                success = self.schema_indexer.index_database_schema(db_connection)
                if success:
                    logger.info("Schema successfully indexed to LanceDB")
                else:
                    logger.warning(
                        "Schema indexing failed, but RAG agent will continue with fallback context"
                    )
            except Exception as e:
                logger.warning(
                    f"Schema indexing failed: {e}, continuing with fallback context"
                )
        elif self.schema_indexer.db is None:
            logger.warning(
                "LanceDB not available, RAG agent will use fallback context retrieval"
            )
        ...
        logger.debug("RAG Agent initialized with self-correction capabilities")
    ...
    def _save_graph_visualization(self):
        ...
        try:
            ...
            logger.debug("RAG workflow graph saved as agents/rag_workflow_graph.png")
            ...
            try:
                ...
                with open("agents/rag_workflow_graph.txt", "w") as f:
                    f.write(graph_ascii)
                logger.debug(
                    "RAG workflow graph saved as agents/rag_workflow_graph.txt"
                )
                ...
                try:
                    ...
                    logger.debug(
                        "RAG workflow description saved as agents/rag_workflow_description.txt"
                    )
